// frontend/src/components/pos/CartItemRow.tsx
    ...
    <div className="p-3 bg-muted/50 rounded-lg border">
      <div className="flex justify-between items-start mb-2">
        <div className="flex-1">
          <p className="font-medium">{product.name} {product.is_weighted && ` - ${weight} ${selectedUnit?.unit_name}`}</p>
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <div>
          <Label className="text-xs">{product.is_weighted ? 'Packets' : 'Quantity'}</Label>
          <div className="flex gap-1">
            <Button
              size="sm"
              variant="outline"
              onClick={() => onQuantityChange(productId, Math.max(product.is_weighted ? 1 : 0.01, quantity - 1))}
            ...
            <Input
              type="number"
              step={product.is_weighted ? "1" : "0.01"}
              value={quantity}
              onChange={(e) => {
